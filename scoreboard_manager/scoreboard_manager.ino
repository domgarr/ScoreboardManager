...
int connectionCheck; // Info on if the sport watch is connected or not.
...
void loop(void)
{
  ...
  /*
   * Check if connected to watch, if so do not allow buttons to function. Although, if the watch is not connected do so. 
   * 
   */

  //Use the AT command to check if the watch is connected.
  if(ble.sendCommandCheckOK("AT+GAPGETCONN")){
       //Grab that value from the buffer.
       connectionCheck = atoi(ble.buffer);
       //Serial.print(connectionCheck);
  }
